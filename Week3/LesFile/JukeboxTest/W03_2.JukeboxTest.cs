...
public class JukeboxTest
{
    ...
    */
    ...
    public void TestSingleDuration()
    {
        ...
        var expected = "2m:0s";
        var actual = jukebox.PlayListDuration();
    ...
    {
        MyJukebox jukebox = new MyJukebox();
        var song = new Song("Best song in the world", "Unkown artist", 120);
        //Song song = jukebox.Playlist[jukebox.Playlist.Count - 1];

        var expected = "'Best song in the world' performed by 'Unkown artist' - 2m:0s";
        var actual = song.Info();
        Assert.AreEqual(expected, actual,
                string.Format("Expected: {0}; Actual: {1}",
                                expected, actual));
    }
    ...
    public void TestAddSongs()
    {
        MyJukebox jukebox = new MyJukebox();
        jukebox.AddSong("Best song in the world", "Unkown artist", 120);
        jukebox.AddSong("Best song in the world, maybe?", "Unkown artist", 210);
        
        var actual = jukebox.HowManySongs() == 2;
    ...
    */
    ...
    public void TestPlayListDuration()
    {
        var ListOfSongs = new List<Song> {new Song("Best song in the world", "Unkown artist", 120),
                                          new Song("Rain in blood", "Slayer", 340)};
        MyJukebox jukebox = new MyJukebox();
        foreach(var song in ListOfSongs)
            jukebox.AddSong(song.Title, song.Artist, song.Duration);

        string expected = "7m:40s";
        var actual = jukebox.PlayListDuration();
        Assert.AreEqual(expected, actual,
                string.Format("Expected: {0}; Actual: {1}",
                                expected, actual));
